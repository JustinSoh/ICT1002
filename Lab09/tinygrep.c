#include <stdio.h>
#include <string.h> 
int strncmp(char * string, char * pattern, char * sens) {
	char dest[255] = "";
	int number = 0;
	...
	if (strcmp(sens, "N") == 0) {
		if (number != 0) {
			//Got dots in front 
			char * ptr = strstr(tolower(string), dest);
			if (ptr != NULL) {
				int pos = ptr - string;
			...
			//No dots
			char * ptr = strstr(tolower(string), pattern);
			if (ptr != NULL) {
				int pos = ptr - string;
				return pos;
			}
...
int main() {
	...
	//5. Compare 
	int pos = strncmp(input, pattern, caseSensitive);
	if (pos != -1) {
